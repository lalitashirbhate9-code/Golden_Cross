const sampleStocks = [
  {name:'N R Agarwal Inds',symbol:'NRAIL',price:584,lastPrice:584,firstPrice:571.4,discoveredDate:'2026-08-24',percentChange:2.2,ma50:571.2,ma200:548.6,change:2.14,pe:15.05,roce:8.49,marketCap:993.92,volume:165937,average:19558,catalyst:'Profit growth +111% · paper demand',sector:'Paper & Packaging',crossed:'12 days ago'},
  {name:'Nexus Select',symbol:'NXST',price:165.89,lastPrice:165.89,firstPrice:163.81,discoveredDate:'2026-08-10',percentChange:1.27,ma50:161.4,ma200:157.8,change:1.27,pe:57.06,roce:5.83,marketCap:25132.34,volume:4966636,average:610789,catalyst:'Sales growth +10.9% · yield 1.47%',sector:'Real Estate',crossed:'26 days ago'},
  {name:'Nitiraj Engineer',symbol:'NITIRAJ',price:219.19,lastPrice:219.19,firstPrice:220.60,discoveredDate:'2026-08-28',percentChange:-0.64,ma50:207.6,ma200:191.2,change:-0.64,pe:93.62,roce:1.79,marketCap:224.69,volume:38842,average:4359,catalyst:'Profit growth +419% · order momentum',sector:'Engineering',crossed:'8 days ago'}
];
...
function parseCsv(text) {
  ...
  return rows.map((row,index)=>{
    ...
    const lastPrice=number(get(row,['lastprice','latestprice']))||price;
    const firstPrice=number(get(row,['firstprice','discoveredprice']))||lastPrice;
    return {name,symbol:get(row,['nsecode','symbol','code'])||name.slice(0,8).toUpperCase(),price:lastPrice,lastPrice,firstPrice,discoveredDate:get(row,['discovereddate','discoverydate'])||new Date().toISOString().slice(0,10),percentChange:firstPrice?(lastPrice/firstPrice-1)*100:0,ma50,ma200,change:number(get(row,['change','changepercent','pricechange']))||0,pe:number(get(row,['pe','peratio']))||null,roce:number(get(row,['roce']))||null,marketCap:number(get(row,['marketcap','marketcapitalisation']))||0,volume:number(get(row,['volume','volume1d']))||0,average:number(get(row,['averagevolume1month','averagevolume1mth','volume1mth']))||0,catalyst:'Imported Screener fundamentals',sector:get(row,['sector'])||'Unclassified',crossed:'Imported row'};
...
function render(){
  ...
  $('#stockRows').innerHTML=visible.map((s,i)=>{const last=s.lastPrice||s.price, first=s.firstPrice||last, performance=s.percentChange??(last/first-1)*100;return `<tr><td><div class="stock"><span class="rank">#${i+1}</span><div><strong>${escapeHtml(s.name)}</strong><span class="symbol">${escapeHtml(s.symbol)} · ${escapeHtml(s.sector)}</span></div></div></td><td class="mono">${money(last)}<small>latest fetched</small></td><td class="mono">${money(first)}<small>first price · ${escapeHtml(s.discoveredDate||'—')}</small></td><td class="mono ${performance>=0?'up':'down'}">${performance>=0?'+':''}${performance.toFixed(2)}%<small>since discovered</small></td><td class="mono">${money(s.ma50)}</td><td class="mono">${money(s.ma200)}</td><td><span class="distance up">+${s.distance.toFixed(1)}%</span><small class="distance">above 200D · ${escapeHtml(s.crossed)}</small></td><td><div class="fundamentals"><span><b>${s.pe?s.pe.toFixed(1):'—'}</b><small>P/E</small></span><span><b>${s.roce?s.roce.toFixed(1)+'%':'—'}</b><small>ROCE</small></span><span><b>${s.marketCap?compact(s.marketCap):'—'}</b><small>CAP</small></span></div></td><td class="mono">${s.volume&&s.average?(s.volume/s.average).toFixed(1)+'x':'—'}<small class="distance">volume ratio</small></td><td class="catalyst">${escapeHtml(s.catalyst)}</td><td><span class="badge ${s.fresh?'buy':'watch'}">${s.fresh?'FRESH CROSS':'ABOVE MA'}</span></td></tr>`}).join('');
